...
func (repo *MessageRepo) GetFullMessages() ([]model.FullMessage, error) {
	...
	rows, err := repo.db.Query("SELECT m.id, m.Title, c.id, c.Name, u.Fullname, u.Photourl, (SELECT COUNT(id) FROM replie WHERE message_id = m.id)  FROM message m LEFT JOIN channel c ON c.id = m.channel_id LEFT JOIN tg_user u ON u.id = m.user_id;")
	if err != nil {
	...
	for rows.Next() {
		...
		err := rows.Scan(&message.ID, &message.Title, &message.ChannelID, &message.ChannelName, &message.FullName, &message.PhotoURL, &message.ReplieCount)
		if err != nil {
			continue
		}
