...
func (h *Handler) savedPage(w http.ResponseWriter, r *http.Request) {
	...
	h.tmpTree["saved"] = template.Must(
		template.ParseFiles(
			"templates/user/saved.html", "templates/partials/navbar.html", "templates/partials/header.html", "templates/message/message.html",
			"templates/message/messages.html", "templates/channel/channel.html", "templates/user/user.html", "templates/channel/channels.html",
			"templates/base.html",
		),
	)
	err = h.tmpTree["saved"].ExecuteTemplate(w, "base", data)
	if err != nil {
		h.log.Error(err)
	}
}
